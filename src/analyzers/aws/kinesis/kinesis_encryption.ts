...
export class StreamEncryptionAnalyzer extends BaseAnalyzer {
    ...
    public analyze(params: any): any {
        ...
        const allStreamsAnalysis: IResourceAnalysisResult[] = [];

        for (const streamName in allStreamEncryptions) {
            const streamEncryption = allStreamEncryptions[streamName];
            ...
            allStreamsAnalysis.push(streamAnalysis);
        }
        stream_encryption.regions = { global: allStreamsAnalysis };
